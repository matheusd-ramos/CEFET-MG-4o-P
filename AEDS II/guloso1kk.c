...
typedef struct {
    ...
    double *distancias;  // Distâncias correspondentes (usaremos como 'chave' para Prim)
    int tamanho;
    int capacidade;
} Heap;
...
void primMST_heap(ListaAdj *grafo, int n) {
    ...
    while (!heapVazia(heap)) {
        ...
        while (atual != NULL) {
            ...
            // Esta é a verificação chave do Prim
            // Se 'v' não está na MST e a aresta (u,v) é mais barata
            // que a aresta 'chave' atual de 'v'
            comparacoes++; // Contando a comparação principal do algoritmo
            if (!incluido[v] && peso < chave[v]) {
                // Atualiza a chave e o pai de 'v'
                chave[v] = peso;
                pai[v] = u;
                // Insere 'v' no heap com sua nova prioridade (peso da aresta)
...
int main() {
    ...
    for (int e = 0; e < m; e++) {
        ...
        // Para Prim, o grafo deve ser NÃO-DIRECIONADO
